...
<colgroup span="2"><col></col><col width="350px"></col></colgroup>
	...
	<tr>
		...
		<?=$face_image?> <?=$name?></b>
		<?if($data['homepage']) {?><img src="<?=$dir?>/images/icon_homepage.gif" class="xe_member_homeIcon" title="홈페이지" alt="홈페이지" /><a href="<?=$data['homepage']?>" target="_blank" title="홈페이지 바로가기">홈페이지</a><?}?></td>
	...
	<tr class="xe_viewFile_tr">
		<td class="xe_viewFile">
			<img src="<?=$dir?>/images/iconReply.gif" style="width:12px; height:12px; vertical-align:middle; margin-top:-4px; margin-right:5px" title="댓글" alt="댓글" />댓글 : <span class="list_com_sp"><?if($comment_num) {?><?=$comment_num?><?}else{?>0<?}?></span>
		</td>
		<td class="xeCom_btn">
			...
			<?if(!eregi($setup[no]."_".$no,$HTTP_SESSION_VARS["zb_vote"])) {// 추천 버튼?>
			<a class="button" href="vote.php?<?=$href?><?=$sort?>&no=<?=$no?>" title="추천"><span>추천</span></a>
			<?}?>
			<?if($is_admin||$member[level]<=$setup[grant_list]) {// 목록 버튼?>
			...
			<?}?>
			<?if(($is_admin||$member[level]<=$setup[grant_reply])&&$no&&$data[headnum]>-2000000000) {// 답글 버튼?>
			<a class="button" href="write.php?<?=$href?><?=$sort?>&no=<?=$no?>&mode=reply&sn1=<?=$sn1?>" title="답글"><span>답글</span></a>
			<?}?>
			<?if($is_admin||$member[level]<=$setup[grant_write]) {// 새글 버튼?>
