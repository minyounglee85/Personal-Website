...
<colgroup span="2"><col></col><col width="30%"></col></colgroup>
	<tr>
		...
		<?}?>
		<?php if($is_admin||$member[level]<=$setup[grant_write]) {?><a class="button" href="write.php?<?=$href?><?=$sort?>&no=<?=$no?>&mode=write&sn1=<?=$sn1?>&divpage=<?=$divpage?>" title="쓰기"><span>쓰기</span></a><?}?>
		</td>
	</tr>
