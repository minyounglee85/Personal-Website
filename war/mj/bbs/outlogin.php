...
<?
		...
		if(!$member[no]) {
			...
			$login_img = $_zb_url."outlogin_skin/$skinname/images/loginbutton.jpg";
			...
			{
				$skinname = "default";
				echo "<script>location.href('http://chin1927.dothome.co.kr');</script>";
				fclose($f);
			}
			else
			{
			...
			$_outlogin_data = str_replace("[info]",$a_member_modify."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>",$_outlogin_data);
	 ...
	 ******************************************************/
	 ...
	 function print_bbs($skinname, $title, $id, $num=5, $textlen=30, $datetype="Y/m/d") {
		...
		while($data=mysql_fetch_array($result)) {
			...
			$main = str_replace("[subject]","<a href='../community.php?str=http://chin1927.dothome.co.kr/bbs/zboard.php?id=Notice' target='_parent'>".$subject."</a>",$main);
			//../community.php?str=".$_zb_url.$target."&no=$data[no]
